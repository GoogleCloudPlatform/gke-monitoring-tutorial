...
def get_args():
    ...
    Configures and runs argparse.ArgumentParser to extract command line arguments.

    Returns:
        An argparse.Namespace containing the arguments parsed from the command line
    """
...
def get_refs():
    """Converts the directory of boilerplate files into a map keyed by file extension.
...
def has_valid_header(filename, refs, regexs):  # pylint: disable=too-many-locals
    """Test whether a file has the correct boilerplate header.

    Tests each file against the boilerplate stored in refs for that file type (based on
    extension), or by the entire filename (eg Dockerfile, Makefile). Some heuristics
    are applied to remove build tags and shebangs, but little variance in header
    ...
    Returns:
        True if the file has the correct boilerplate header, otherwise returns False.
    """
    try:
    ...
    years_to_be_replaced = regexs["date"]
    for idx, datum in enumerate(data):
        (data[idx], found) = years_to_be_replaced.subn('YEAR', datum)
        if found != 0:
            break
...
def normalize_files(files):
    """Extracts the files that require boilerplate checking from the files argument.

    A new list will be built. Each path from the original files argument will be added
    unless it is within one of SKIPPED_DIRS. All relative paths will be converted
    to absolute paths by prepending the root_dir path parsed from the command line, or
    its default value.
    ...
    Returns:
        A modified copy of the files list where any any path in a skipped directory is
        removed, and all paths have been made absolute.
    """
    newfiles = []
...
def get_files(extensions):
    ...
    If a list of file names has been provided on the command line, it will be treated
    as the initial set to search. Otherwise, all paths within rootdir will be discovered
    and used as the initial set.

    Once the initial set of files is identified, it is normalized via normalize_files()
    and further stripped of any file name whose extension is not in extensions.

    Args:
        extensions: a list of file extensions indicating which file types should have
                    their boilerplate verified
...
def get_regexs():
    ...
    There are two scenarios where these regexes are used. The first is in validating the
    date referenced is the boilerplate, by ensuring it is an acceptable year. The second
    is in identifying non-boilerplate elements, like shebangs and compiler hints that
    should be ignored when validating headers.
...
def main():
    ...
    If all files contain correct boilerplate, this function terminates normally.
    Otherwise it prints the name of each non-conforming file and
